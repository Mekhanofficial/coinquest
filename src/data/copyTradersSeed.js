...
const buildName = (index) => {
  ...
  const last = LAST_NAMES[Math.floor(index / FIRST_NAMES.length)];
  return `${first} ${last}`;
};
...
const buildImageUrl = (index) => {
  ...
  return `https://picsum.photos/seed/coinquest-copy-${index + 1}/320/320`;
};

export const COPY_TRADERS_SEED = Array.from({ length: 100 }, (_, index) => {
  const id = index + 1;
