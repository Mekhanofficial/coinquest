...
export function TransactionProvider({ children }) {
      ...
      if (response.ok) {
        ...
        if (result.success && result.data) {
          const apiTransactions = result.data.map(tx => ({
            ...
            network: tx.network,
            details: tx.details || `${tx.type} ${tx.currency} via ${tx.paymentMethod}`,
          ...
          const pendingCount = apiTransactions.filter(tx => tx.status === "Pending").length;
  ...
  const forceRefreshTransactions = async () => {
    ...
    return await fetchTransactionsFromAPI(true);
  };
  ...
  const refreshTransactions = async (maxRetries = 3, delay = 2000) => {
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      ...
      const result = await fetchTransactionsFromAPI(true); // Force refresh
      
      if (result.success) {
        setLastTransactionCount(result.count);
        
        // Also refresh user balance to ensure consistency
        try {
          await refreshUser();
